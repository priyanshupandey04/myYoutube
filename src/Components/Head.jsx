...
import { GOOGLE_SEARCH_SUGGESTIONS_API } from "../Utils/constants";
import { addSearchQuery } from "../Utils/SearchQuery";
...
const Head = () => {
  ...
  const getSearchSuggestions = async () => {
    const response = await fetch(GOOGLE_SEARCH_SUGGESTIONS_API + searchQuery);
    const data = await response.json();
    setSearchSuggestions(data[1]);

    dispatch(addSearchQuery({ searchQuery, searchSuggestions: data[1] }));
  };
      ...
      <div className="flex items-center justify-center col-span-10 ">
        <div className="relative w-5/12 h-full ">
          <div className="flex h-full">
            <input
              className="items-center w-full h-full px-2 border-2 border-gray-400 rounded-l-full"
              type="text"
              placeholder="Search"
              value={searchQuery}
            ...
            />
            <button className="h-full px-3 bg-gray-200 border-2 border-gray-400 rounded-r-full">
              <svg
          ...
          <div
            className={`absolute z-10 w-full ${
              searchSuggestions.length > 0 ? "py-2 border border-gray-300" : ""
            } bg-white  h-fit`}
          >
            {searchSuggestions.map((searchRes, index) => (
              <div
                key={index}
                className="flex h-10 px-2 py-2 mx-2 font-medium rounded-md hover:bg-gray-200 hover:cursor-pointer"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
